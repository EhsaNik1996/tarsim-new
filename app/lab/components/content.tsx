  ...
  return (
    <section className="flex mx-auto px-7 gap-4 max-w-7xl max-sm:flex-col max-sm:px-4">
      {steps.map((step, index) => {
        ...
        return (
          <article
            className={`group flex flex-1 flex-col relative h-96 overflow-hidden border transition duration-500 hover:-translate-y-3 hover:shadow-2xl p-8 rounded-3xl max-sm:h-64 ${active ? "bg-linear-to-br from-accent to-cyan text-white border-white/20" : "bg-white/65 border-line backdrop-blur-lg hover:border-accent"}`}
            key={step.label}
          >
            <i
            ...
            <div className="flex relative items-center justify-between">
              <span className="font-sans text-xs opacity-60">0{index + 1}</span>
